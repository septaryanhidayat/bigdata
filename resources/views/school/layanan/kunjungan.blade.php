...
<head>
    ...
    <title>Permohonan Izin Kunjungan Sekolah | {{ $settings['school_name'] }}</title>
...
<body class="bg-slate-50 text-slate-900 antialiased min-h-screen pb-24">
    ...
    <main class="py-10 max-w-5xl mx-auto px-4 space-y-8">
        ...
        <div class="text-center space-y-3">
            ...
            <h1 class="text-3xl sm:text-4xl font-black text-slate-900 tracking-tight">Permohonan Izin Kunjungan ke Sekolah</h1>
            <p class="text-slate-600 text-xs sm:text-sm font-medium max-w-2xl mx-auto">
                Pengajuan kunjungan studi banding, riset observasi, silaturahmi instansi, maupun rombongan edukasi ke SIT Robbani Ogan Ilir.
            </p>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 p-2 rounded-2xl bg-slate-200/70 border border-slate-300 shadow-inner">
            <a href="{{ route('school.layanan.kunjungan') }}" class="p-3.5 rounded-xl border text-center font-black text-xs flex items-center justify-center gap-2 transition-all tab-active">
        ...
        <div class="card-light-surface p-6 sm:p-8 rounded-[2rem] space-y-6">
            ...
            <div class="flex items-center gap-3 border-b border-slate-200 pb-4">
                ...
                <div>
                    ...
                    <p class="text-xs text-slate-500 font-semibold">Ketentuan resmi pengajuan permohonan kunjungan ke SIT Robbani</p>
                </div>
            </div>
                ...
                <div class="p-5 rounded-2xl bg-emerald-50/60 border border-emerald-200 space-y-3">
                    ...
                    <ul class="space-y-2 text-slate-700 font-medium pl-1">
                        <li class="flex items-start gap-2">
                            ...
                            <span>Pemohon memiliki akun pada sistem untuk melakukan permohonan kunjungan.</span>
                        </li>
                        <li class="flex items-start gap-2">
                        ...
                        <li class="flex items-start gap-2">
                            ...
                            <span>Bukti permohonan kunjungan sudah ditandatangani oleh pejabat berwenang &amp; stempel basah dibawa ketika hari kunjungan.</span>
                        </li>
                        <li class="flex items-start gap-2">
                ...
                <div class="space-y-4">
                    ...
                    <div class="p-5 rounded-2xl bg-orange-50/60 border border-orange-200 space-y-3">
                        ...
                        <div class="border-t border-orange-200/80 pt-2.5">
                            ...
                            <p class="text-xs font-bold text-slate-900 mt-0.5">Permohonan Izin Kunjungan ke Sekolah</p>
                        </div>
                    </div>
        ...
        <div class="p-5 rounded-2xl bg-emerald-600 text-white font-black text-sm space-y-1 shadow-lg flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-xs font-bold text-emerald-100">Tim Humas SIT Robbani akan memproses permohonan Anda paling lambat 10 hari kerja.</p>
            </div>
        </div>
        @endif

        <!-- Form Section -->
        <form action="{{ route('school.layanan.kunjungan.store') }}" method="POST" class="card-light-surface p-6 sm:p-8 rounded-[2rem] space-y-5">
            @csrf
            
            <div class="border-b border-slate-200 pb-3 flex items-center justify-between">
                <h3 class="text-base font-black text-slate-900 flex items-center gap-2">
                    <span>📝</span> <span>Isi Formulir Permohonan Kunjungan</span>
                </h3>
                <span class="text-[10px] text-slate-400 font-bold">* Wajib diisi</span>
            </div>
            
            <div class="space-y-1.5">
                <label class="block text-xs font-black text-emerald-800 uppercase">Nama Instansi / Sekolah / Lembaga *</label>
                <input type="text" name="instansi" required placeholder="Contoh: SDIT Nurul Islam / Universitas / Instansi Dinas" class="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-300 text-xs font-semibold text-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-600 focus:bg-white">
            </div>

            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="space-y-1.5">
                    <label class="block text-xs font-black text-emerald-800 uppercase">Nama Penanggung Jawab *</label>
                    <input type="text" name="nama_pemohon" required placeholder="Nama lengkap & gelar penanggung jawab" class="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-300 text-xs font-semibold text-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-600 focus:bg-white">
                </div>
                <div class="space-y-1.5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="space-y-1.5">
                    <label class="block text-xs font-black text-emerald-800 uppercase">Email Aktif *</label>
                    <input type="email" name="email" required placeholder="email@instansi.com" class="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-300 text-xs font-semibold text-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-600 focus:bg-white">
                </div>
                <div class="space-y-1.5">
                    <label class="block text-xs font-black text-emerald-800 uppercase">Rencana Tanggal Kunjungan (Senin / Kamis) *</label>
                    <input type="date" name="tgl_kunjungan" required class="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-300 text-xs font-semibold text-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-600 focus:bg-white">
                </div>
            </div>

            <div class="space-y-1.5">
                <label class="block text-xs font-black text-emerald-800 uppercase">Estimasi Jumlah Peserta (Maksimal 100 Orang) *</label>
                <input type="number" name="jumlah_peserta" max="100" min="1" required placeholder="Contoh: 25" class="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-300 text-xs font-semibold text-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-600 focus:bg-white">
            </div>

            <div class="space-y-1.5">
                <label class="block text-xs font-black text-emerald-800 uppercase">Maksud & Tujuan Kunjungan *</label>
                <textarea name="tujuan" rows="4" required placeholder="Jelaskan agenda, topik studi banding, atau maksud kunjungan..." class="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-300 text-xs font-semibold text-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-600 focus:bg-white"></textarea>
            </div>
