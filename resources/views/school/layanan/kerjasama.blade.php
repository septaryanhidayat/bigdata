...
<body class="bg-[#0b1206] text-lime-50 antialiased min-h-screen pb-24 lg:pb-0">
    ...
    <header class="bg-[#070c04] py-4 px-6 sticky top-0 z-50 border-b border-[#1c3011]">
        <div class="max-w-7xl mx-auto flex items-center justify-between">
            <a href="{{ route('home') }}" class="flex items-center gap-3">
                ...
                <div>
                    <span class="font-black text-xs block text-[#a8f52c] uppercase">PERMOHONAN KERJASAMA</span>
    ...
    <main class="py-12 max-w-3xl mx-auto px-4 space-y-8">
        ...
        <div class="text-center space-y-3">
            <span class="px-3.5 py-1 rounded-full bg-[#14220c] border border-[#264218] text-[#a8f52c] font-black text-xs uppercase">KEMITRAAN & SINERGI</span>
            <h1 class="text-3xl font-black text-white">Form Pengajuan Kerjasama Lembaga</h1>
            <p class="text-slate-400 text-sm font-medium">Pengajuan kemitraan program pendidikan, sponsorship kegiatan, media partner, atau sinergi dakwah.</p>
        </div>
        ...
        <form action="{{ route('school.layanan.kerjasama.store') }}" method="POST" class="p-8 rounded-[2rem] card-dark-surface space-y-5">
            @csrf
            
            <div class="space-y-1.5">
                <label class="block text-xs font-black text-[#a8f52c] uppercase">Nama Perusahaan / Lembaga / Mitra *</label>
                <input type="text" name="nama_lembaga" required placeholder="PT / Yayasan / Komunitas" class="w-full px-4 py-3 rounded-xl bg-[#070c04] border border-[#264218] text-xs font-semibold text-white focus:outline-none focus:border-[#a8f52c]">
            </div>

            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="space-y-1.5">
                    <label class="block text-xs font-black text-[#a8f52c] uppercase">Nama Penanggung Jawab *</label>
                    <input type="text" name="nama_kontak" required placeholder="Nama lengkap" class="w-full px-4 py-3 rounded-xl bg-[#070c04] border border-[#264218] text-xs font-semibold text-white focus:outline-none focus:border-[#a8f52c]">
                </div>
                <div class="space-y-1.5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="space-y-1.5">
                    ...
                    <input type="email" name="email" required placeholder="mitra@perusahaan.com" class="w-full px-4 py-3 rounded-xl bg-[#070c04] border border-[#264218] text-xs font-semibold text-white focus:outline-none focus:border-[#a8f52c]">
                </div>
                <div class="space-y-1.5">
                    <label class="block text-xs font-black text-[#a8f52c] uppercase">Kategori Kerjasama *</label>
                    <select name="jenis_kerjasama" required class="w-full px-4 py-3 rounded-xl bg-[#070c04] border border-[#264218] text-xs font-semibold text-white focus:outline-none focus:border-[#a8f52c]">
            ...
            <div class="space-y-1.5">
                ...
                <textarea name="deskripsi" rows="4" required placeholder="Jelaskan penawaran kerjasama, bentuk sinergi, dan manfaat bersama..." class="w-full px-4 py-3 rounded-xl bg-[#070c04] border border-[#264218] text-xs font-semibold text-white focus:outline-none focus:border-[#a8f52c]"></textarea>
            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            Swal.fire({
                icon: 'success',
                title: 'Pengajuan Kerjasama Terkirim!',
