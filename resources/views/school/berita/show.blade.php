...
<head>
    ...
    <style>
        ...
        /* Any images inside tables or lists should be constrained */
    ...
    <header class="sticky top-0 z-50 transition-colors duration-300 bg-white/90 dark:bg-[#07170a]/90 backdrop-blur-md border-b border-slate-200 dark:border-[#1a3d1e] shadow-xs">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 py-3.5 flex items-center justify-between">
            <a href="{{ route('home') }}" class="flex items-center gap-3 group">
                ...
                <div>
                    <span class="font-black text-xs block text-emerald-700 dark:text-[#c6f634] uppercase tracking-wider group-hover:text-emerald-600">PORTAL BERITA RESMI</span>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <article class="lg:col-span-8 space-y-6">
                ...
                <div class="bg-white dark:bg-[#07170a] rounded-3xl overflow-hidden border border-slate-200 dark:border-[#1a3d1e] shadow-md">
                    ...
                    <div class="p-3.5 bg-slate-50 dark:bg-[#0a1f0e] text-[11px] text-slate-600 dark:text-slate-300 text-center font-bold border-t border-slate-200 dark:border-[#1a3d1e] flex items-center justify-center gap-1.5">
                ...
                <!-- Interactive Social Share Action Bar (Proporsional) -->
                <div class="p-5 sm:p-6 rounded-3xl bg-emerald-50/80 dark:bg-[#07170a] border border-emerald-200 dark:border-[#1a3d1e] flex flex-col sm:flex-row items-center justify-between gap-4 shadow-sm">
                    <div class="space-y-0.5 text-center sm:text-left">
                        ...
                        <span class="text-[11px] text-slate-500 dark:text-slate-400 font-medium">Bantu sebarkan informasi positif kegiatan santri Robbani</span>
                    </div>
                    <div class="flex flex-wrap items-center justify-center gap-2">
                        <a href="https://api.whatsapp.com/send?text={{ urlencode($news['title'] . ' ' . request()->fullUrl()) }}" target="_blank" class="px-4 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-xs shadow-xs transition-transform hover:scale-105 flex items-center gap-1.5">
                            <span>💬 WhatsApp</span>
                        </a>
                        <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(request()->fullUrl()) }}" target="_blank" class="px-4 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-extrabold text-xs shadow-xs transition-transform hover:scale-105 flex items-center gap-1.5">
                            <span>📘 Facebook</span>
                        </a>
                        <button onclick="navigator.clipboard.writeText(window.location.href); alert('Link berita berhasil disalin ke clipboard!');" class="px-4 py-2.5 rounded-xl bg-slate-800 dark:bg-[#0c2210] hover:bg-slate-900 dark:hover:bg-[#143319] text-white dark:text-[#c6f634] border border-slate-700 dark:border-[#1a3d1e] font-extrabold text-xs shadow-xs transition-transform hover:scale-105 flex items-center gap-1.5">
                ...
                <!-- Author Bio Card -->
                <div class="bg-white dark:bg-[#07170a] p-5 sm:p-6 rounded-3xl border border-slate-200 dark:border-[#1a3d1e] shadow-sm flex items-start gap-4">
                    <div class="w-12 h-12 rounded-2xl bg-emerald-700 text-white font-black flex items-center justify-center text-sm shadow-md shrink-0">
                        {{ strtoupper(substr($news['author'] ?? 'H', 0, 2)) }}
                    </div>
                    <div class="space-y-1 min-w-0">
                        <div class="flex items-center gap-2">
                            <h4 class="font-extrabold text-sm text-slate-900 dark:text-white">{{ $news['author'] ?? 'Humas SIT Robbani' }}</h4>
                            <span class="px-2 py-0.5 rounded-md bg-emerald-100 dark:bg-[#0c2210] text-emerald-800 dark:text-[#c6f634] text-[10px] font-black uppercase">Official Publisher</span>
                        </div>
                        <p class="text-xs text-slate-600 dark:text-slate-400 leading-relaxed font-medium">
                            Dipublikasikan oleh Tim Media &amp; Publikasi Resmi SIT Robbani Ogan Ilir untuk mengabarkan kemajuan akademik, karakter islami, dan prestasi santri.
                        </p>
                    </div>
                </div>
                ...
                <div class="pt-4 border-t border-slate-200 dark:border-[#1a3d1e] flex flex-col sm:flex-row items-center justify-between gap-3">
                    <a href="{{ route('school.berita') }}" class="w-full sm:w-auto px-5 py-3 rounded-2xl bg-white dark:bg-[#07170a] border border-slate-200 dark:border-[#1a3d1e] text-slate-800 dark:text-slate-200 font-extrabold text-xs hover:border-emerald-500 hover:text-emerald-700 dark:hover:text-[#c6f634] transition-all shadow-xs flex items-center justify-center gap-2">
            ...
            <aside class="lg:col-span-4 space-y-6 lg:sticky lg:top-24">
                ...
                <div class="bg-white dark:bg-[#07170a] p-5 rounded-3xl border border-slate-200 dark:border-[#1a3d1e] shadow-sm space-y-4">
                    ...
                    <div class="space-y-3">
                        ...
                        <a href="{{ route('school.berita.show', $rn['slug'] ?? \Illuminate\Support\Str::slug($rn['title'])) }}" class="flex items-center gap-3 group p-2 rounded-2xl hover:bg-slate-50 dark:hover:bg-[#0a1f0e] border border-transparent hover:border-slate-200 dark:hover:border-[#1a3d1e] transition-all">
                ...
                <div class="bg-white dark:bg-[#07170a] p-5 rounded-3xl border border-slate-200 dark:border-[#1a3d1e] shadow-sm space-y-3">
                    <h4 class="font-black text-xs text-slate-900 dark:text-white uppercase tracking-wider flex items-center gap-1.5 border-b border-slate-100 dark:border-[#143319] pb-3">
                        <span>📅</span> Agenda &amp; Event Terdekat
                    </h4>

                    <div class="space-y-2.5">
                        @foreach(array_slice($agendaList ?? [], 0, 3) as $agenda)
                        <div class="flex items-center gap-3 p-2.5 rounded-2xl bg-slate-50 dark:bg-[#0a1f0e] border border-slate-200/80 dark:border-[#1a3d1e]">
                            <div class="flex flex-col items-center justify-center bg-emerald-700 text-white w-11 h-11 rounded-xl shrink-0 font-bold shadow-xs">
                                <span class="text-xs leading-none font-black">{{ $agenda['date_day'] ?? '15' }}</span>
                ...
                <div class="bg-white dark:bg-[#07170a] p-5 rounded-3xl border border-slate-200 dark:border-[#1a3d1e] shadow-sm space-y-3">
                    ...
                    <div class="space-y-2.5 text-xs">
                        ...
                        <div class="p-3 rounded-2xl bg-slate-50 dark:bg-[#0a1f0e] border border-slate-200/80 dark:border-[#1a3d1e] space-y-1 hover:border-emerald-500 transition-all">
                ...
                <div class="bg-white dark:bg-[#07170a] p-5 rounded-3xl border border-slate-200 dark:border-[#1a3d1e] shadow-sm space-y-3">
                    ...
                    <div class="space-y-2 text-xs font-bold text-slate-700 dark:text-slate-300">
                        <a href="https://wa.me/62811747472" target="_blank" class="flex items-center gap-2.5 p-2 rounded-xl bg-slate-50 dark:bg-[#0a1f0e] hover:text-emerald-600 transition-colors">
                            <span class="text-emerald-600">💬</span> <span>0811-7474-72 (WhatsApp)</span>
                        </a>
                        <a href="mailto:info@sitrobbani.sch.id" class="flex items-center gap-2.5 p-2 rounded-xl bg-slate-50 dark:bg-[#0a1f0e] hover:text-emerald-600 transition-colors truncate">
                            <span class="text-blue-600">✉️</span> <span class="truncate">info@sitrobbani.sch.id</span>
                        </a>
                        <div class="flex items-center gap-2.5 p-2 rounded-xl bg-slate-50 dark:bg-[#0a1f0e]">
                            <span class="text-orange-600">📍</span> <span>Indralaya, Ogan Ilir, Sumsel</span>
                        </div>
                    </div>
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            proseImages.forEach(function(img) {
                // If image has intrinsic or styled small dimensions, apply inline icon class
