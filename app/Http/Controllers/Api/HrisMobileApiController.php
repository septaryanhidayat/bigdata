...
class HrisMobileApiController extends Controller
{
    ...
    /**
     * Pendaftaran & Pembaruan Sampel Wajah Biometrik (Face ID Enrollment)
     */
    public function enrollFace(Request $request)
    {
        $userId = $request->header('X-User-Id') ?? 1;
        $user = User::find($userId) ?? User::first();
        $employee = $this->getEffectiveEmployee($user);
        $empId = $employee ? $employee->id : ($user ? $user->id : 1);

        $faceImage = $request->input('face_image') ?? $request->input('photo') ?? $request->input('avatar');
        ...
        $filename = 'face_employee_' . $empId . '.jpg';
        ...
        if ($faceImage) {
            ...
            } elseif (strlen($faceImage) > 200 && !str_starts_with($faceImage, 'http')) {
                $decoded = base64_decode($faceImage);
                if ($decoded) {
                    file_put_contents($fullPath, $decoded);
                }
            } elseif (str_starts_with($faceImage, 'http')) {
                $photoUrl = $faceImage;
            }
        }
        ...
        return response()->json([
            'status' => 'success',
            'message' => 'Sampel wajah biometrik (Face ID) berhasil didaftarkan dan tersimpan permanen.',
            'face_photo_url' => $photoUrl,
            'face_registered_at' => date('d M Y, H:i') . ' WIB',
            'user' => [
                'id' => $user ? $user->id : 1,
                'avatar' => $photoUrl,
            ],
    ...
    /**
     * Cek Status Pendaftaran Wajah Biometrik (Face Status)
     */
    public function faceStatus(Request $request)
    {
        $userId = $request->header('X-User-Id') ?? 1;
        ...
        $isEnrolled = $employee && !empty($employee->face_registered_at);
        $faceUrl = $employee ? ($employee->face_photo_url ?? ($user ? $user->avatar : null)) : ($user ? $user->avatar : null);

        return response()->json([
            'status' => 'success',
            'is_face_registered' => (bool)$isEnrolled,
            'face_photo_url' => $faceUrl,
            'face_registered_at' => $employee && $employee->face_registered_at ? date('d M Y, H:i', strtotime($employee->face_registered_at)) . ' WIB' : null,
            'employee_name' => $employee ? $employee->full_name : ($user ? $user->name : 'Pegawai SIT Robbani'),
            'nip' => $employee ? ($employee->nip ?? '-') : '-',
            'unit_name' => 'SIT Robbani',
        ]);
    }
}
